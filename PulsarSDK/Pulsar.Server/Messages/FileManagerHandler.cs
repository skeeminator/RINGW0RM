...
namespace Pulsar.Server.Messages
{
    ...
    public class FileManagerHandler : MessageProcessorBase<string>, IDisposable
    {
        ...
        private void Execute(ISender client, FileTransferChunk message)
        {
            ...
            // If first chunk and we don't know size yet
            ...
            if (fullyDone)
            {
                ...
                // Notify UI BEFORE removing transfer
                OnFileTransferUpdated(transfer);
        ...
        // SERVER - when client says "I'm done"
        private void Execute(ISender client, FileTransferComplete message)
        {
            ...
            // If we didn't actually receive all bytes, treat as error
            if (transfer.Size > 0 && transfer.TransferredSize < transfer.Size)
            {
                transfer.Status = "Error: incomplete transfer (FileTransferComplete before size reached)";
                OnFileTransferUpdated(transfer);
